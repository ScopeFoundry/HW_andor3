...
from ScopeFoundryHW.andor3 import andor3_consts as consts
...
class Andor3(object):
    ...
    def __init__(self, debug = False, initialize_to_defaults=True):
        ...
        if platform.architecture()[0] == '64bit':
            andorlibpath = r"C:\Program Files\Andor SOLIS\atmcd64d_sdk3.dll"
        ...
        if self.debug:  logger.debug("Andor3 initializing")
            
        with self.lock: _err(self.andorlib.Initialize(''))
        if self.debug: logger.debug("Andor 3 Library Initialization Successful")
